<template>
  <div class="talking">
    <header>
      <i @click="$router.back(-1)"><font-awesome-icon class="leftArrow" :icon="['fas', 'angle-left']" /></i>
      <h1>{{friend.userId}}</h1>
    </header>

    <div class="talkingContent">
      <ul>
        <li v-for="msgInfo in this.$store.getters.getChatMsg(this.friend.userId)">

          <div class="myselfMsg contactsInfo" v-if="msgInfo.fromUser == user.data.userId">
            <span class="contacts">
...
<style lang="scss" scoped>
  header{position: fixed; top: 0; left: 0; width: 100%; z-index: 2; color:#fff; background-color: #38383d; height: $size44; font-size: $size18;
    i{ position: absolute; left: $size10; top: 0; font-size: $size16; line-height: $size44;
